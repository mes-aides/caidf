...
const CONDITION_STATEGY = {
  benefit: {
    test: (condition, { openfiscaResponse, periods }) => {
      let benefit = openfiscaResponse.individus.demandeur[condition.name] || openfiscaResponse.familles._[condition.name]
      let eligibility = Boolean(benefit[periods.thisMonth.id])
  ...
  formation_sanitaire_social: {
    test: (condition, { situation }) => {
      return situation.demandeur._formationSanitaireSocial === condition.value
    },
  },
